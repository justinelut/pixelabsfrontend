...
export default function ProductPricing({ pricing, saletype, saleroute, pid, slug }){
    return (
            ...
            <div className="grid max-w-md gap-10 row-gap-5 sm:row-gap-10 lg:max-w-screen-md lg:grid-cols-2 sm:mx-auto">    
                {pricing && pricing.map(pricelist=>(
                    <div className="flex flex-col justify-between p-5 bg-white border rounded shadow-sm">
                        <div className="mb-6">
                            <div className="flex items-center justify-between pb-6 mb-6 border-b">
                                <div>
                                    ...
                                    <p className="text-5xl font-extrabold"> {currency.format(pricelist.price)}</p>
                                </div>
                                ...
                                <ul className="space-y-2">
                                    {
                                        pricelist.details.map(detail=>(
                                            <li className="flex items-center">
                                                ...
                                                <p className="font-medium text-gray-800">{detail.Details}</p>
                                            </li>
                                        ))
                                    }
                                </ul>
                        ...
                        <div>
                            <Link
                                href={`/checkout?p=${pricelist.type}&s=${slug}&cat=${saletype}&srt=${saleroute}&id=${pid}&plidx=${pricelist.id}`}
